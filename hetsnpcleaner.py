...
#    cmd = 'cat %s | java -Xmx6G -jar /home/ian/galaxy-dist/tools/snpEff/SnpSift.jar filter "( DP > 2 ) & ( DP < 32 ) & ( QUAL >= 10 ) ! (REF = %s) ! (ALT = %s)" > %s' %(tmpf0vcf_name,blah,blah,options.output1)
    cmd = 'cat %s |java -Xmx4G -jar /home/galaxy/galaxy-dist/tools/snpEff/SnpSift.jar filter "( DP > 2 ) & ( DP < 32 ) & ( QUAL >= 10 ) & ( DP4[0] > 0 ) & ( DP4[1] > 0 ) & ( DP4[2] > 0 ) & ( DP4[3] > 0 ) ! (REF = %s) ! (ALT = %s)" > %s' %(tmpf0vcf_name,blah,blah,options.output1) # cleaned het SNPs read direction corrected
